...
1. **DO NOT use parallel worktrees / `openspec-oneshot` / `Agent`
   ...
   branch.**
2. **Backend interpreter is fixed:** `/path/to/deskpet/backend/.venv/Scripts/python.exe`
   (Windows; git-bash forward-slash paths). Run pytest with this exact
   path from `backend/`.
3. **Runtime config ≠ repo config.** Backend reads
   `C:\Users\you\AppData\Roaming\deskpet\config.toml` (a.k.a.
...
1. `cd backend && /path/to/deskpet/backend/.venv/Scripts/python.exe -m pytest tests/ -q --ignore=tests/test_deskpet_vector_worker.py` → 0 regressions; `cd tauri-app && npx tsc --noEmit` + `npm test`.
2. Clean single-stack restart (kill zombies first). Dev stack launch
   ...
   $env:DESKPET_BACKEND_DIR="/path/to/deskpet\backend";
   $env:DESKPET_DEV_ROOT="/path/to/deskpet";
   Set-Location /path/to/deskpet\tauri-app; npm run tauri dev
   ```
