...
- **GIVEN** SessionDB has 2 code sessions, "vpn-tunnel" pinned to "relay", "research" with no binding
...
- **THEN** ws response includes:
  ```json
  {
    ...
    "sessions": [
      {"base_session_id": "vpn-tunnel", ..., "provider_id": "relay", "preferred_model": null},
...
- **THEN** `code_mode_state` ws event includes the saved `provider_id="relay"` + `preferred_model=null`
- **AND** UI dropdown defaults to "relay" (NOT "Global Chain")
...
- **THEN** returns `{"provider_id": "relay", "preferred_model": None}`
