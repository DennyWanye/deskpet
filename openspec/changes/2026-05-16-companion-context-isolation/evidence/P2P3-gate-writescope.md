...
- ✅ Scenario "Companion session blocked from writing into a code repo"：`write_file({path:"G:\\projects\\deskpet\\backend\\vpn-cli\\...", _write_scope_root:<ws>})` → `{ok:false}`，文案 = spec 原文，文件未创建。
