...
1. **NO sandbox / permission walls** ([feedback_no_sandbox_constraints](C:/Users/24378/.claude/projects/G--projects-deskpet/memory/feedback_no_sandbox_constraints.md))
...
2. **Real test, not just unit tests** ([feedback_real_test](C:/Users/24378/.claude/projects/G--projects-deskpet/memory/feedback_real_test.md), [feedback_simulate_manual_test](C:/Users/24378/.claude/projects/G--projects-deskpet/memory/feedback_simulate_manual_test.md))
...
3. **No Tauri dev orphans** ([feedback_tauri_dev_cleanup](C:/Users/24378/.claude/projects/G--projects-deskpet/memory/feedback_tauri_dev_cleanup.md))
...
```bash
cd /g/projects/deskpet/backend && python -m pytest tests/ -q --tb=line --ignore=tests/test_deskpet_vector_worker.py
```
...
```bash
cd /g/projects/deskpet/backend && python -m pytest tests/test_p5s2_<topic>.py -v --tb=short
```
...
```bash
cd /g/projects/deskpet/tauri-app && npm test
```
...
```bash
cd /g/projects/deskpet/tauri-app && npx tsc --noEmit
```
...
cd /g/projects/deskpet/tauri-app && \
  ...
  npm run tauri:dev > /g/projects/deskpet/tauri-dev.log 2>&1 &

# 4. Wait for "Application startup complete"
until grep -qE "Application startup complete" /g/projects/deskpet/tauri-dev.log; do sleep 3; done

# 5. Grep new shared secret
grep "secret=" /g/projects/deskpet/tauri-dev.log | tail -1
```
...
```bash
grep -E "<event_name>|<error_class>" /g/projects/deskpet/tauri-dev.log | tail -10
```
...
Live config at `%LocalAppData%\deskpet\config.toml` (user-writable). Source-of-truth schema at `/g/projects/deskpet/config.toml`. Adding a new key:
...
```bash
# Backend
cd /g/projects/deskpet/backend && python -m pytest tests/ -q --tb=line --ignore=tests/test_deskpet_vector_worker.py 2>&1 | tail -5

# Frontend tests
cd /g/projects/deskpet/tauri-app && npm test 2>&1 | tail -10

# TypeScript
cd /g/projects/deskpet/tauri-app && npx tsc --noEmit 2>&1 | tail -5
```
...
```bash
cd /g/projects/deskpet && openspec archive <change-name>
```
