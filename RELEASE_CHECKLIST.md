...
# 2. chinzy.com 没漏到公开源（auth/* 例外见 oss-checks.yml 注释）
git grep -c 'chinzy\.com' -- '*.py' '*.ts' '*.tsx' '*.rs' '*.toml' '*.json' \
...
- 早期 commit message 提到的 `chinzy` / 真实密码字符串
...
chinzy.com==>your-llm-relay.example.com
chinzy==>relay
EOF
)
...
| 3 | chinzy 默认 + 破损 identifier + 2 个内部文档 untrack | `aef897f` |
