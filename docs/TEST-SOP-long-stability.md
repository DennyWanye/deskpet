...
**流式层**（实时）：
- `tail -fn0 /g/projects/deskpet/tauri-dev.log | grep --line-buffered 真错误模式`
- 任何匹配立即 alert，不等周期结束
...
sleep 3
cd tauri-app && DESKPET_BACKEND_DIR=/g/projects/deskpet/backend npm run tauri:dev > /g/projects/deskpet/tauri-dev.log 2>&1 &
until grep -qE "Application startup complete" /g/projects/deskpet/tauri-dev.log; do sleep 5; done

# 3. 启用 auto-mode
SEC=$(grep "secret=" /g/projects/deskpet/tauri-dev.log | tail -1 | grep -oE 'secret=[a-f0-9]+' | cut -d= -f2)
python -c "
...
```bash
# real errors
grep -aE "(Traceback|Exception:|hallucination|chat_v2_error|circuit_breaker_open|all_providers_failed|UnboundLocalError|TypeError|KeyError|AttributeError|chat_persist_.*_failed)" /g/projects/deskpet/tauri-dev.log | \
  grep -avE "test_p6|known_extras|chinese_diagnostic|args_malformed|IPC custom|ws_closed_midstream|WebSocketDisconnect" | wc -l

# activity metrics
echo "tool_calls: $(grep -ac p5s2_tool_call_args_dump /g/projects/deskpet/tauri-dev.log)"
echo "end_turns:  $(grep -ac \"stop_reason='end_turn'\" /g/projects/deskpet/tauri-dev.log)"
