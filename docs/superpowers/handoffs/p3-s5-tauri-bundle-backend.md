...
Test-Path /path/to/deskpet\.claude\worktrees\p3-s5-tauri-bundle-backend\backend\dist\deskpet-backend\deskpet-backend.exe
# 期望 True

# 2. 模型目录要在
Test-Path /path/to/deskpet\backend\models\faster-whisper-large-v3-turbo
# 期望 True
...
```powershell
cd /path/to/deskpet\.claude\worktrees\p3-s5-tauri-bundle-backend
