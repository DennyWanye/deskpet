...
| P2-G4 | 多角色切换时长 | 前端 profiler | < 500 ms（不包含 LLM warm-up） |
...
| 多角色协作 | `ServiceContext` N 实例化 | P2-1 |
...
- `PersonaRegistry` —— 挂多 `ServiceContext` 的注册表 + 会话绑定
...
### 3.2 Sprint P2-1 — 云端混合推理 & 多角色基线（目标版本 `v0.3.0`）

**目标：** 上线云端 LLM fallback + 第一版多 persona 切换。

**为什么合并这两条？** 多角色 (A5) 需要 `ServiceContext` 可多实例化，而云端混合 (A6) 的配置按 persona 作用域最自然 —— 同一个 session 想用离线 / 联网的不同模型，本质都是 "换 provider"。放一起做能共用 PersonaRegistry 抽象。
...
| P2-1-S4 PersonaRegistry | `agent/persona_registry.py` —— key 到 `ServiceContext` 的映射；会话绑定 persona_id | persona.json 配置格式；默认自带 3 个 persona（本地桌宠 / 联网助手 / 代码助手） |
| P2-1-S5 前端 Persona 切换 | `PersonaSwitcher.tsx` 组件 —— 头部下拉 + 活跃 persona 高亮 | 切换时清空 DialogBar、保留 memory 独立 |
...
- **D1-4**：persona 配置文件放哪？`%APPDATA%/deskpet/personas/*.toml` 还是内置 JSON？
...
| `v0.3.0-phase2-beta2` | 云端混合 + 多角色 | P2-1 |
...
**P2-1 前：** ~~D1-1（云厂选型）~~ ✅ DashScope 百炼 · D1-1a（qwen 模型档位）· D1-2（默认策略）· D1-3（预算）· D1-4（persona 配置路径）· D1-5（首启引导）
