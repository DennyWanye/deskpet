...
export function MessageStreamPanel({
  ...
  if (collapsed) {
    return (
      <button
        type="button"
        data-testid="msgstream-expand"
        title="展开消息流"
        onClick={onExpand}
        style={embedded ? embeddedCollapsedStyle : collapsedHandleStyle}
        aria-label="展开消息流"
      >
        ▸
      </button>
  ...
  return (
    ...
    >
      <header style={headerStyle}>
        ...
        <button
          type="button"
          data-testid="msgstream-collapse"
          onClick={onCollapse}
          title="收起"
          aria-label="收起消息流"
          style={closeBtnStyle}
...
// 2026-05-16: embedded variant — code-mode 窗口 3 列 flex 里用。视觉与
// 上面一致，仅去掉 absolute 定位/硬编 top·left·width，改为填满父 flex
// 格（父格给定宽高）。桌宠窗口 embedded=false 走上面的 absolute，零回归。
...
const embeddedCollapsedStyle: CSSProperties = {
  position: "relative",
  width: 18,
  height: "100%",
  borderRight: "1px solid rgba(148, 163, 184, 0.18)",
