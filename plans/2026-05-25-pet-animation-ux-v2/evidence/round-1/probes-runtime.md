...
| **CASE-D0-05** LLM emotion field | `the relay.chat("…")` returns dict with `emotion` key | **FAIL** — no `llm.the relay` module; LLM goes through `backend/llm/openai_adapter.py` / `gemini_adapter.py`. System prompt does not append `emotion` field. | **FAIL → fallback to frontend `emotionClassifier` voting** |
